...
fr_in = img_param.fr_n;                             % input frame number
smpls_fr = size(RcvData, 1) / fr_in - 128;          % samples per frame
fr_out = floor(fr_in / img_param.fr_d) - n_ang + 1; % output frame number
...
RF_demod = RF_mas .* ...
    repmat(exp(- pi * 1i / 2 * (0:size(RF_mas, 1)-1)'), [1, size(RF_mas, 2, 3)]);
