function [u_true, mean_u]= interp_real_u(...
    est_x, est_z, times, PData, lambda, ct_list)
...
u_true = zeros([size(x_grid), length(times), length(ct_list)]);
mean_u = zeros(length(ct_list), 1);

for c_t = 1:length(ct_list)

    % Load displacement info from .h5 file
    [rea_t, rea_x, ~, rea_z, ~, ~, u_z] = load_u(...
        sprintf('../resources/FemData/u_%.0f.h5', 3e3 * ct_list(c_t)^2));
    
    mean_u(c_t) = mean(u_z, 'all') / lambda;
    ...
    for i = 1:length(times)
        ...
        u_pts = interp2(rea_z, rea_x, u_intp, z_grid(:), x_grid(:), ...
            'linear', 0);
        u_true(:, :, i, c_t) = reshape(u_pts, size(x_grid)) / lambda;
    end
end

% Squeeze singleton dimensions
u_true = squeeze(u_true);
